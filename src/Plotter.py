...
def plot_curves(curves: tuple[np.ndarray], labels: list[str], peaks: Optional[list[int]], name: Optional[str] = None) -> None:
    ...
    if int(x_len[1]) <= 4:
        x_max = x_len[0] + \
            str(int(x_len[1])+(5-int(x_len[1]))) + '0'*len(x_len[2:])
    else:
    ...
    color_list = ['#e31a1c', '#1f77b4', '#33a02c',
                  '#6a3d9a', '#ff7f00', '#b15928']

    fig = plt.figure(figsize=(8.5, 4))
    fig.subplots_adjust(
        right=0.85 - (0.1 * max(len(curves)-2, 0)), bottom=0.25)
    base_ax = plt.axes()
    ...
    while not good:
        ...
        for i, ax in enumerate(curves):
            ubound, lbound = ything * \
                round(max(ax)/ything), ything * round(min(ax)/ything)
            ...
            if len([x for x in range(lower, upper+ything, ything)]) > yticks_len:
                yticks_len = len(
                    [x for x in range(lower, upper+ything, ything)])
        if yticks_len < 6:
            yticks_len *= 2
            ything = ything // 2
        else:
    ...
    handle_list = []
    for i, ax in enumerate(curves):
        # y refers to the y-axis coordinates, not the y-curve
        peaks_y = np.asarray([ax[j] for j in peaks])
        ax_list[i].plot(range(len(ax)), ax, color=color_list[i],
                        zorder=2, label=labels[i])
        ax_list[i].scatter(peaks, peaks_y, marker='o', c='k',
                           zorder=3, label='$\it{oriC}$')
        ax_list[i].tick_params(axis='y', colors=color_list[i])
        ax_list[i].ticklabel_format(
    ...
    if name is not None:
        base_ax.set_title(f'2D Z-Curve: {name}',
    ...
    base_ax.ticklabel_format(axis='x', style='sci',
